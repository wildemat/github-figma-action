...
async function main() {
  ...
  const { FIGMA_TOKEN: figmaToken, GITHUB_TOKEN: githubToken, PR_NUMBER: prNumber, GITHUB_REPOSITORY } = process.env;
  ...
  try {
    ...
    const prBody = await githubApi.fetchCurrentPRBody(owner, repo, prNumber, githubToken);
    ...
    console.log(`Found ${specsAnalysis.existingSpecCount} existing figma link(s) in Design Specs section`);
    ...
    console.log(`Found ${figmaLinks.length} Figma link(s) above Design Specs section`);
    ...
    for (let i = 0; i < figmaLinks.length; i++) {
      try {
        const result = await processFigmaLink(figmaLinks[i], specsAnalysis.existingSpecCount + i + 1, figmaToken);
        specsContent += result.specSnippet;
        updatedBody = updatedBody.replace(figmaLinks[i].fullMatch, result.referenceText);
        console.log(`Processed Figma link ${i + 1}/${figmaLinks.length}: ${figmaLinks[i].url}`);
      } catch (error) {
        console.error(`Error processing Figma link ${figmaLinks[i].url}:`, error.message);
      }
    }

    // Update Design Specs section
    if (specsContent) {
      updatedBody = updateDesignSpecsSection(updatedBody, specsContent, specsAnalysis);
      console.log(`Added ${figmaLinks.length} design spec(s) to Design Specs section`);
    }

    // Update PR if changes were made
    if (updatedBody !== prBody) {
      await updatePRDescription(updatedBody, prBody, owner, repo, prNumber, githubToken);
    } else {
...
function analyzeDesignSpecsSection(prBody) {
  ...
  if (hasSpecsSection) {
    ...
    const existingFigmaLinks = specsSectionContent.match(regexPatterns.EXISTING_FIGMA_LINKS_REGEX);
...
function findFigmaLinks(prBody, specsAnalysis) {
  ...
  while ((markdownMatch = markdownRegex.exec(contentToSearch)) !== null) {
    const linkText = markdownMatch[1];
    const fullUrl = markdownMatch[2];
    const ids = figmaApi.extractFigmaIds(fullUrl);
  ...
  while ((standaloneMatch = standaloneRegex.exec(contentToSearch)) !== null) {
...
async function processFigmaLink(link, specNumber, figmaToken) {
  ...
  const existingVersionId = figmaApi.extractVersionId(link.url);
  let version;

  if (existingVersionId) {
    console.log(`Using existing version from URL: ${existingVersionId}`);
    version = figmaApi.createVersionFromId(existingVersionId);
  } else {
    version = await figmaApi.fetchLatestVersion(link.fileId, figmaToken);
  }

  // Get image URL and download
  const imageUrl = await figmaApi.fetchNodeImageUrl(link.fileId, link.nodeId, figmaToken);
  console.log(`Using Figma image URL (expires in 30 days): ${imageUrl}`);
  
  // We still need to download to trigger the API, but we use the URL directly
  await figmaApi.downloadImageData(imageUrl);

  // Generate content
  const specId = `design-spec-${specNumber}`;
  const expirationString = utils.calculateImageExpirationDate();
  const cleanUrl = utils.createCleanFigmaUrl(link.fileId, link.nodeId, version.id);
...
function updateDesignSpecsSection(body, specsContent, specsAnalysis) {
  ...
  if (specsAnalysis.hasSpecsSection) {
    if (specsAnalysis.specsEndIndex > specsAnalysis.specsSectionIndex) {
      ...
      return body.substring(0, specsAnalysis.specsEndIndex) +
             specsContent +
      ...
      const nextSectionMatch = afterSpecsSection.match(regexPatterns.NEXT_SECTION_REGEX);

      if (nextSectionMatch) {
        const nextSectionIndex = specsAnalysis.specsSectionIndex + nextSectionMatch.index;
        const withEndMarker = body.substring(0, nextSectionIndex) +
                             `\n${endMarker}` +
                             body.substring(nextSectionIndex);
        
        const newEndMarkerIndex = withEndMarker.indexOf(endMarker);
        return withEndMarker.substring(0, newEndMarkerIndex) +
               specsContent +
 ...
 */
async function updatePRDescription(updatedBody, originalBody, owner, repo, prNumber, githubToken) {
  ...
  console.log(`Body diff: ${updatedBody.length - originalBody.length} characters added`);

  const status = await githubApi.updatePRDescription(owner, repo, prNumber, updatedBody, githubToken);
